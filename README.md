...
- **AI Runtime Evidence Protocol (AIREP)** — [`ai-runtime-evidence-protocol`](https://github.com/halvrenofviryel/ai-runtime-evidence-protocol): a vendor-neutral, *experimental* open format for a per-decision **AI decision receipt** — one signed, hash-chained, offline-checkable record per AI runtime decision (record groups: subject, input, claim, output, evidence, directive, scope, integrity), verifiable by two independent verifiers (Python + Node) over RFC 8785 canonical JSON. It is a *proposed* format, not a ratified standard. The Phionyx **Reasoned Governance Envelope (RGE)** is AIREP's reference producer — the first system to emit AIREP records — and is a Phionyx profile of AIREP.
