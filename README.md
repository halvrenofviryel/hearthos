...
| **Evidence** (the notary) | `apps/demo/lib/phionyx/envelope.ts`: each step becomes a hash-linked `bounded_authority_envelope` record with a public-keyed replay HMAC; `verifyChain()` re-derives the hash links and detects un-rechained edits — it does not verify the HMAC and does not provide producer authenticity or adversarial tamper resistance; `verifyBoundedAuthority()` checks 7 preservation rules. | Any module — download the audit chain `.jsonl`, change a byte, re-verify; the chain names the broken step. |
...
- The browser demo provides a deterministic hash-linked integrity demonstration and a public-keyed replay HMAC. It detects un-rechained edits, but does not provide producer authenticity or adversarial tamper resistance.
